...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($category)) $errors[] = 'カテゴリーは必須です。';

    if (empty($errors)) {
        // Format date properly (e.g. 2023-01-01) - if empty allow null
        $compDate = !empty($completion_date) ? $completion_date : null;

        $result = SupabaseClient::insert('works', [
            'title' => $title,
